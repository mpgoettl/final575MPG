...
function setMap(){
    ...
    var promises = [];
    promises.push(d3.csv("data/reportcardES.csv")); //load attributes from csv
    promises.push(d3.json("data/background.topojson")); //load background spatial data
    promises.push(d3.json("data/esBound.topojson")); //load choropleth spatial data
    ...
    function callback(data){
        ...
        var UScountries = topojson.feature(usCountries, usCountries.objects.background),
            usSTATES = topojson.feature(USA, USA.objects.esBound).features;
...
function joinData(usSTATES, csvData){
    ...
    for (var i=0; i<csvData.length; i++){
        ...
        var csvKey = csvRegion.Schl_Code; //the CSV primary key

        //loop through geojson states to find correct state
        for (var a=0; a<usSTATES.length; a++){

            var geojsonProps = usSTATES[a].properties; //the current state geojson properties
            var geojsonKey = geojsonProps.Schl_Code; //the geojson primary key
...
function setEnumerationUnits(usSTATES, map, path, colorScale){
    ...
    var regions = map.selectAll(".regions")
        ...
        .attr("class", function(d){
            return "regions " + d.properties.Schl_Code;
...
function changeAttribute(attribute, csvData){
    ...
    updateChart(csvData.length, colorScale);//bars,
};
...
function highlight(props){
    //change stroke
    var selected = d3.selectAll("." + props.Schl_Code)
        .style("stroke", "blue")
...
function setLabel(props){
    ...
    var infolabel = d3.select("body")
        ...
        .attr("id", props.Schl_Code + "_label")
        .html(labelAttribute);

    var regionName = infolabel.append("div")
        .attr("class", "labelname")
        .html(props.SchoolName);
